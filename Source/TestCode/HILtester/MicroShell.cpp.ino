...
char* SetParameters(int TestTime, int Period, float Tolerance);
char* Start();


// Put here function wrapper

void Start_wrapper(int argc, char **argv)
{
  #ifdef DEBUG
  printf("Result: %s\n", Start());
  #else
...
void SetParameters_wrapper(int argc, char **argv)
{
  int a;
  int b;
  int c;
  argvscanf("%d %d %d", &a, &b, &c);
  #ifdef DEBUG
  printf("Result: %s\n", SetParameters(a, b, (float) c/100.0));
  #else
  SetParameters(a, b, (float) c/100.0);  
  #endif
}
...
{
  {"Start", Start_wrapper},
  {"SetParameters", SetParameters_wrapper},
  {"", 0}
};
